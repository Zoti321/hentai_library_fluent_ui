      ...
      <div className="w-full max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-2 duration-500">
        <header className="mb-8 flex items-end justify-between">
            <div>
                ...
                <p className="text-gray-500 text-sm mt-1">Good afternoon, Reader</p>
            </div>
            <div className="flex gap-2">
                <button onClick={onRefresh} className="p-2 bg-white text-gray-600 border border-border-subtle rounded-md hover:bg-gray-50 hover:text-primary transition-all shadow-sm active:scale-95" title="Refresh">
                ...
                <button className="flex items-center gap-2 bg-primary text-white px-4 py-2 rounded-md font-medium text-sm hover:bg-primary-hover shadow-sm transition-all active:scale-95">
                    <Plus className="w-4 h-4" /><span>Scan Library</span>
                </button>
            </div>
        </header>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
            <div className="bg-white p-6 rounded-xl border border-border-subtle shadow-fluent flex flex-col justify-between h-40">
                <div className="flex items-start justify-between">
                    <div>
                        <div className="text-gray-500 text-xs font-semibold uppercase tracking-wider">Collection</div>
                ...
                <div className="text-xs text-gray-500"><span className="text-green-600 font-medium">+3 new</span> this week</div>
            </div>
            <div className="col-span-1 md:col-span-2 bg-gradient-to-br from-primary to-primary-hover text-white p-6 rounded-xl shadow-fluent relative overflow-hidden h-40 flex items-center">
                <div className="relative z-10 w-full">
                    <div className="text-white/80 text-xs font-semibold uppercase tracking-wider mb-2">Featured Pick</div>
                    <h3 className="text-2xl font-semibold mb-1">Discover Something New</h3>
                    <p className="text-white/70 text-sm mb-0">Explore the latest fantasy additions to your library.</p>
                </div>
        ...
        {continueReadingList.length > 0 && (
            <section className="mb-10">
                <div className="flex items-center justify-between mb-4">
                    <h2 className="font-semibold text-lg text-gray-900">Continue Reading</h2>
                    <button className="text-sm text-primary hover:underline flex items-center gap-1">View all <ChevronRight className="w-3 h-3"/></button>
                </div>
                    ...
                    {continueReadingList.map(manga => (
                        <div key={manga.id} className="bg-white p-3 rounded-xl border border-border-subtle shadow-sm hover:shadow-fluent transition-all cursor-default group flex items-center gap-3" onClick={() => onMangaClick(manga)} onContextMenu={(e) => onContextMenu(e, manga)}>
                            ...
                            <div className="flex-1 min-w-0">
                                ...
                                <div className="text-xs text-gray-500 mt-1">Chapter 5</div>
        ...
        <section>
            <h2 className="font-semibold text-lg text-gray-900 mb-4">Recently Added</h2>
